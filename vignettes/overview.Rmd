...
`r Biocpkg("scuttle")` provides some useful utilities for developers of downstream packages.
This includes `.subset2index()` to convert a row/column indexing vector into an integer vector for safer downstream use;
`.splitRowsByWorkers()` and `.splitColsByWorkers()`, to split a dataset into submatrices that will be sent to each worker;
and `.bpNotSharedOrUp()`, which indicates whether a `r Biocpkg("BiocParallel")` backend should be re-started.
