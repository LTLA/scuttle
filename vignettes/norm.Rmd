...
In this case, we use the `quickCluster()` function from the `r Biocpkg("scran")` package, 
but any clustering function or prior subpopulation information can be used here.
...
The `logNormCounts()` function calculates log~2~-transformed normalized expression values by dividing each count by the size factor for the cell, adding a constant pseudo-count and log-transforming.
